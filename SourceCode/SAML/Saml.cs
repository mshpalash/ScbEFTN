...
namespace FloraSoft
{
	...
	public partial class Response
	{
		...
		public Response(string certificateStr, string responseString) : this(StringToByteArray(certificateStr), responseString) { }
		...
		public void LoadXml(string xml)
		{
            ...
            SamlLogManager.WriteLog("RESPONSE : " + _xmlDoc.OuterXml);
		...
		public bool IsValid()
		{
            ...
            var ck1 = ValidateSignatureReference(signedXml);
            //var ck2 = signedXml.CheckSignature(_certificate, true);
            var ck3 = !IsExpired();

            //SamlLogManager.WriteLog("ValidateSignatureReference : " + ck1 + "; CheckSignature : " + ck2 + "; IsExpired : " + !ck3);
            //return ck1 && ck2 && ck3;

            SamlLogManager.WriteLog("ValidateSignatureReference : " + ck1 + "; IsExpired : " + !ck3);
            return ck1 && ck3;
		}
    ...
    public class AuthRequest
    {
        ...
        public virtual string GetRequest(AuthRequestFormat format)
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                SamlLogManager.WriteLog("AuthRequest : " + sw.ToString());
        ...
        public virtual string GetRedirectUrl(string samlEndpoint, string relayState = null)
        {
    ...
    public class ScbAuthRequest : AuthRequest
    {
        private string _destination;

        public ScbAuthRequest(string issuer, string assertionConsumerServiceUrl) : base(issuer, assertionConsumerServiceUrl)
        {            
        }

        public override string GetRequest(AuthRequestFormat format)
        {
            using (StringWriterWithEncoding sw = new StringWriterWithEncoding(Encoding.UTF8))
            {
                ...
                using (XmlWriter xw = XmlWriter.Create(sw, xws))
                {
                    ...
                    //Issuer - Entity Name
                    xw.WriteStartElement("saml", "Issuer", "urn:oasis:names:tc:SAML:2.0:assertion");
                    xw.WriteAttributeString("Format", "urn:oasis:names:tc:SAML:2.0:nameid-format:kerberos");
                    xw.WriteString(_issuer);
                    xw.WriteEndElement();
                ...
                if (format == AuthRequestFormat.Base64)
                {
                    ...
                    //https://stackoverflow.com/questions/25120025/acs75005-the-request-is-not-a-valid-saml2-protocol-message-is-showing-always%3C/a%3E
                    var memoryStream = new MemoryStream();
                    var writer = new StreamWriter(new DeflateStream(memoryStream, CompressionMode.Compress, true), new UTF8Encoding(true));
                    writer.Write(sw.ToString());
                    writer.Close();
                    string result = Convert.ToBase64String(memoryStream.GetBuffer(), 0, (int)memoryStream.Length, Base64FormattingOptions.None);
                    return result;
        ...
        public override string GetRedirectUrl(string samlEndpoint, string relayState = null)
        {
            _destination = samlEndpoint.Split(new[] { '?' })[0];

            return base.GetRedirectUrl(samlEndpoint, relayState);
        }
    }
